...
class PlayerConfigurationRuntimeTest {
    @Test
    fun recreationRestoresRetryablePlaybackSurface() {
        ...
        ActivityScenario.launch<PlayerActivity>(intent).use { scenario ->
            ...
            val initialText = viewText(scenario, PlayerActivity.ERROR_VIEW_ID)
            assertTrue("initial error must be non-empty", initialText.isNotBlank())

            scenario.recreate()

            await("recreated error surface") {
                viewVisibility(scenario, PlayerActivity.ERROR_VIEW_ID) == View.VISIBLE &&
                    viewVisibility(scenario, PlayerActivity.RETRY_BUTTON_ID) == View.VISIBLE
            }
            assertTrue("error message must survive recreation", viewText(scenario, PlayerActivity.ERROR_VIEW_ID).isNotBlank())

            scenario.onActivity { activity ->
                activity.findViewById<View>(PlayerActivity.RETRY_BUTTON_ID).performClick()
            }
            await("retry surface after recreation") {
                viewVisibility(scenario, PlayerActivity.ERROR_VIEW_ID) == View.VISIBLE &&
                    viewVisibility(scenario, PlayerActivity.RETRY_BUTTON_ID) == View.VISIBLE
            }
            assertTrue("retry must remain available after recreation", viewText(scenario, PlayerActivity.ERROR_VIEW_ID).isNotBlank())
        }
    }
