...
class PaginatedContentLoaderTest {
    ...
    @Test fun `refresh then load more accumulates pages`() = runBlocking {
    ...
    @Test fun `empty first page becomes explicit empty state`() = runBlocking {
        val loader = PaginatedContentLoader { Page(emptyList(), it, false) }
        assertEquals(PaginatedContentLoader.State.Empty, loader.refresh())
    }

    @Test fun `retryable provider failure preserves previous items and retry succeeds`() = runBlocking {
        var failSecond = true
        val loader = PaginatedContentLoader { page ->
