...
class PaginatedContentLoaderTest {
    ...
    @Test fun `refresh then load more accumulates pages`(): Unit = runBlocking {
    ...
    @Test fun `empty first page becomes explicit empty state`(): Unit = runBlocking {
        val loader = PaginatedContentLoader { Page(emptyList(), it, false) }
        assertEquals(PaginatedContentLoader.State.Empty, loader.refresh())
    }

    @Test fun `retryable provider failure preserves previous items and retry succeeds`(): Unit = runBlocking {
        var failSecond = true
        val loader = PaginatedContentLoader { page ->
    ...
    @Test fun `cancellation propagates instead of becoming an error`(): Unit = runBlocking {
