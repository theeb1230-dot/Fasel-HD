...
class PaginatedContentLoaderTest {
    ...
    @Test
    fun `refresh then load more accumulates pages`() {
        runBlocking {
    ...
    @Test
    fun `empty first page becomes explicit empty state`() {
        runBlocking {
    ...
    @Test
    fun `retryable provider failure preserves previous items and retry succeeds`() {
        runBlocking {
            var failSecond = true
            val loader = PaginatedContentLoader { page ->
    ...
    @Test
    fun `cancellation propagates instead of becoming an error`() {
        runBlocking {
